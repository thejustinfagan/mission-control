...
describe("buildMissionControlSnapshot", () => {
  it("produces a fully serializable JSON snapshot", async () => {
    const snapshot = await buildMissionControlSnapshot({ now: NOW, probeTargets: [], localTargets: [], skipGithub: true, skipRender: true });
  ...
  it("exposes every required top-level field", async () => {
    const s = await buildMissionControlSnapshot({ now: NOW, probeTargets: [], localTargets: [], skipGithub: true, skipRender: true });
    for (const key of [
      "generatedAt",
      "globalStatus",
      "summary",
